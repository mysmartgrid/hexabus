...
int main(int argc, char** argv)
{
  ...
  if(okay) {
    ...
    // this map stores all device descriptors. The ones which have an HBA file with a state machine generated are removed from this map,
    // leaving the ones without a state machine for the generation of HBA files without a state machine (just changing the device name).
    hexabus::device_table devices_to_do = *tableBuilder.get_device_table();
    ...
    for(std::map<std::string, hexabus::graph_t_ptr>::iterator graphIt = graphs.begin(); graphIt != graphs.end(); graphIt++) {
      ...
      devices_to_do.erase(graphIt->first);
    }
    ...
    if(vm.count("output")) {
      for(hexabus::device_table::iterator it = devices_to_do.begin(); it != devices_to_do.end(); ++it) {
        ...
        ofs << "# Generated by Hexabus Compiler" << std::endl << std::endl;
        ofs << "target ";
