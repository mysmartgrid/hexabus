...
int main(int argc, char** argv)
{
	...
	if(vm.count("devfile"))
	{
		...
		bool r = false;
		std::ifstream in(vm["devfile"].as<std::string>().c_str(), std::ios_base::in);
		if(verbose)
			std::cout << "Reading input file " << vm["devfile"].as<std::string>() << "..." << std::endl;

		if(!in) {
			std::cerr << "Error: Could not open input file: " << vm["devfile"].as<std::string>() << std::endl;
			return 1;
		}
		...
		pos_iterator_type position_begin(fwd_begin, fwd_end, vm["devfile"].as<std::string>());
		...
		if(r && position_begin == position_end) {
			if(verbose)
				std::cout << "Parsing of file " << vm["devfile"].as<std::string>() << " succeeded." << std::endl;

			/* TODO This is the code for writing the EPDefinitions to the file.
