...
struct PropreportCallback {
	...
	void operator()(const hexabus::Packet& packet, const boost::asio::ip::udp::endpoint asio_ep)
	{
		...
		do {
			...
			const hexabus::PropertyReportPacket<boost::array<char, HXB_16BYTES_PACKET_BUFFER_LENGTH> >* propertyReport_16b = dynamic_cast<const hexabus::PropertyReportPacket<boost::array<char, HXB_16BYTES_PACKET_BUFFER_LENGTH> >* >(&packet);
			...
			const hexabus::PropertyReportPacket<boost::array<char, HXB_65BYTES_PACKET_BUFFER_LENGTH> >* propertyReport_65b = dynamic_cast<const hexabus::PropertyReportPacket<boost::array<char, HXB_65BYTES_PACKET_BUFFER_LENGTH> >* >(&packet);
			if(propertyReport_65b !=NULL) {
				pd.propid = current_pid;
				pd.datatype = HXB_DTYPE_65BYTES;
				pd.value = std::string(propertyReport_65b->value().begin(), propertyReport_65b->value().end());
				current_pid = propertyReport_65b->nextid();
				current_ep->properties.insert(pd);
				break;
			}
		} while(false);
	...
	for(std::set<boost::asio::ip::address_v6>::iterator address_it = addresses.begin(); address_it != addresses.end(); ++address_it)
	{
		...
		do {
			...
			boost::signals2::connection c3 = network->onPacketReceived(boost::ref(propreportCallback),
				...
				hexabus::filtering::isPropertyReport<boost::array<char, HXB_16BYTES_PACKET_BUFFER_LENGTH> >() ||
				hexabus::filtering::isPropertyReport<boost::array<char, HXB_65BYTES_PACKET_BUFFER_LENGTH> >());
