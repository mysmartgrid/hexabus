...
public:
	...
	void transmissionCallback(const hexabus::Packet& packet, uint16_t seqNum, const boost::asio::ip::udp::endpoint asio_ep, bool failed) {
		...
		#ifdef DEBUG
		std::cout << "Failed distributions: " << failcnt << " (" << resetcnt << " resets)" << std::endl;
		#endif
	}
	...
	void infoCallback(const hexabus::Packet& packet, const boost::asio::ip::udp::endpoint asio_ep) {
		...
		if(packet.flags()&HXB_FLAG_RELIABLE) {
			...
			if (auto* info_b = dynamic_cast<const InfoPacket<bool>* >(&packet)) {
				...
				distribution_queue.push(Packet::Ptr(new auto(pinfo)));
			} else if(auto* info_65b = dynamic_cast<const InfoPacket<std::array<uint8_t, 65> >* >(&packet)) {
				...
				if(verbose)
					std::cout << "Unknown data type, ignoring." << std::endl;
				return;
			}
