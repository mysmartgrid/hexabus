...
void DS18X20_show_id_uart( uint8_t *id, size_t n )
{
	...
	for( i = 0; i < n; i++ ) {
		...
		if ( i == 0 ) {
			...
			else if ( id[0] == DS1822_FAMILY_CODE ) { PRINTF (" (22)"); }
			else { PRINTF (" ( ? )"); }
		}
	}
	...
	uint8_t th, uint8_t tl, uint8_t conf)
{
	...
	if( ow_input_pin_state() ) { // only send if bus is "idle" = high
		...
		PRINTF( "DS18X20_write_scratchpad: Short Circuit!\r" );
		ret = DS18X20_ERROR;
	}
...
uint8_t DS18X20_read_scratchpad( uint8_t id[], uint8_t sp[], uint8_t n )
{
	...
	if( ow_input_pin_state() ) { // only send if bus is "idle" = high
		...
		PRINTF( "DS18X20_read_scratchpad: Short Circuit!\r" );
		ret = DS18X20_ERROR;
	}
	...
	uint8_t id[] )
{
	...
	if( ow_input_pin_state() ) { // only send if bus is "idle" = high
		...
		PRINTF( "DS18X20_copy_scratchpad: Short Circuit!\r" );
		ret = DS18X20_START_FAIL;
	}
...
uint8_t DS18X20_eeprom_to_scratchpad( uint8_t id[] )
{
	...
	if( ow_input_pin_state() ) { // only send if bus is "idle" = high
		...
		if ( retry_count ) {
			...
			PRINTF( "DS18X20_recall_E2: timeout!\r" );
			ret = DS18X20_ERROR;
		}
	} 
	else { 
		PRINTF( "DS18X20_recall_E2: Short Circuit!\r" );
		ret = DS18X20_ERROR;
	}
